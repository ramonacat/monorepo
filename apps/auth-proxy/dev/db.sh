...
start() {
	...
	psql --dbname postgres --host="$(pwd)" -c "CREATE ROLE red_proxy WITH LOGIN PASSWORD 'red_proxy'"
	psql --dbname postgres --host="$(pwd)" -c 'CREATE DATABASE red_proxy OWNER red_proxy'
}
