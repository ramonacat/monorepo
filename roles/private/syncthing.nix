...
let
  ...
  current-host-topology =
    lib.mapAttrs (_: hosts: lib.filterAttrs (k: _: k != config.networking.hostName) hosts)
      (
  ...
  settings =
    ...
    // (
      if (builtins.hasAttr config.networking.hostName syncthing-data.settings) then
        (builtins.getAttr config.networking.hostName syncthing-data.settings)
      else
        { }
    );
  connected-devices = lib.filterAttrs (k: _: k != config.networking.hostName) (
    lib.mergeAttrsList (lib.mapAttrsToList (_: machines: machines) current-host-topology)
  );
in
lib.mkIf enable {
  age.secrets."${config.networking.hostName}-syncthing-cert.age".file =
    ../../secrets + "/${config.networking.hostName}-syncthing-cert.age";
  age.secrets."${config.networking.hostName}-syncthing-key.age".file =
    ../../secrets + "/${config.networking.hostName}-syncthing-key.age";
  services.syncthing = {
