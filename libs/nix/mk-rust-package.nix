...
let
  ...
  // (
    if (builtins.isAttrs additional-package-arguments) then
      additional-package-arguments
    else
      (additional-package-arguments { src = filteredSrc; })
  );
  buildAdditionalPackageArgumentsRealised =
    if builtins.isAttrs build-additional-package-arguments then
      build-additional-package-arguments
    else
      (build-additional-package-arguments { src = filteredSrc; });
  cargoArtifacts = crane-lib.buildDepsOnly packageArguments;
  packageArgumentsWithArtifacts = packageArguments // {
...
rec {
  ...
  coverage =
    let
      rawCoverage = crane-lib.cargoLlvmCov (
        { cargoLlvmCovExtraArgs = "--lcov --output-path $out"; } // packageArgumentsWithArtifacts
      );
    in
